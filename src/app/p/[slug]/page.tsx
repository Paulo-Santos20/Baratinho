...
export default async function ProductPage({
  ...
  // Dados Simulados para o Gráfico de Histórico (Em um cenário real, viria do banco)
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="bg-white rounded-[2.5rem] p-4 border border-slate-100 shadow-sm mb-8 flex flex-col md:flex-row gap-4">
              ...
              <div className="flex-1 bg-slate-50 rounded-[2rem] aspect-square md:aspect-auto md:h-[500px] flex items-center justify-center p-8 order-1 md:order-2 relative group overflow-hidden border border-slate-100/50">
                <img 
                  src={deal.imagemUrl || "https://placehold.co/800x800"} 
                  alt={deal.titulo} 
            ...
            {/* HISTÓRICO DE PREÇOS (O Gráfico Caseiro de Alta Performance) */}
                ...
                {historicoPrecos.map((item, index) => {
                  ...
                  return (
                    <div key={index} className="flex flex-col items-center flex-1 group">
                      ...
                      {/* A Barra do Gráfico */}
                      <div 
                        className={`w-full max-w-[3rem] rounded-t-lg transition-all duration-500 ${item.atual ? 'bg-orange-500 shadow-lg shadow-orange-200' : 'bg-slate-100 group-hover:bg-slate-200'}`}
                        style={{ height: `${alturaBarra}%` }}
                      ></div>
                      {/* Mês */}
                      <span className={`text-xs font-bold mt-3 ${item.atual ? 'text-orange-500' : 'text-slate-400'}`}>
                        {item.mes}
                      </span>
                    </div>
                  );
            ...
            <div id="comentarios" className="bg-white rounded-[2rem] p-6 md:p-8 border border-slate-100 shadow-sm">
              ...
              <div className="flex gap-4">
                <div className="w-12 h-12 bg-slate-100 rounded-full flex items-center justify-center flex-shrink-0 text-slate-400 font-black">
                  U
                </div>
                <div className="flex-1">
                  <textarea 
                    placeholder="O que você achou dessa oferta? Vale a pena?"
                    className="w-full bg-slate-50 border border-slate-100 rounded-2xl p-4 outline-none focus:bg-white focus:border-orange-500 transition-all resize-none min-h-[100px]"
                  ></textarea>
                  <div className="flex justify-end mt-2">
                    <button className="bg-slate-900 text-white font-bold px-6 py-3 rounded-xl hover:bg-slate-800 transition-colors">
                      Publicar
                    </button>
                  </div>
                </div>
            ...
            <div className="bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-xl shadow-slate-200/50 sticky top-28">
              ...
              <div className="bg-slate-50 rounded-2xl p-6 mb-6 border border-slate-100">
                {temDesconto && (
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-xs font-bold text-green-600 bg-green-100 px-2 py-1 rounded-md">
                      Economia de {formatarMoeda(valorEconomia)}
                    </span>
                  </div>
                )}
                ...
                <p className="text-sm text-slate-500 font-medium mt-2">
                  Vendido e entregue por <strong className="text-slate-700">{deal.loja || "Loja Oficial"}</strong>
                </p>
              </div>
              ...
              <div className="space-y-3 mb-6">
                <a 
                  ...
                  className="w-full bg-orange-500 text-white font-black text-lg py-5 rounded-[1.5rem] hover:bg-orange-600 transition-all shadow-xl shadow-orange-200 flex items-center justify-center gap-3 active:scale-[0.98]"
                >
              ...
              <div className="flex items-start gap-3 p-4 bg-green-50/50 rounded-2xl border border-green-100 text-green-700">
                ...
                <p className="text-sm font-medium">
                  <strong>Compra Segura:</strong> O Baratinho verifica todos os links. Você será redirecionado para o site oficial da loja.
                </p>
              </div>
