        ...
        <div className="max-w-4xl mx-auto">
          ...
          <form onSubmit={saveDeal} className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-slate-100 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <Input label="Título do Produto" icon={<ShoppingBag size={18}/>} placeholder="Ex: iPhone 15 Pro" value={form.titulo} onChange={(v) => setForm({...form, titulo: v})} />
              <Input label="Loja" icon={<ImageIcon size={18}/>} placeholder="Amazon, Magalu..." value={form.loja} onChange={(v) => setForm({...form, loja: v})} />
              <Input label="Preço Atual" type="number" placeholder="0.00" value={form.preco} onChange={(v) => setForm({...form, preco: v})} />
              <Input label="Preço Antigo" type="number" placeholder="0.00" value={form.precoAntigo} onChange={(v) => setForm({...form, precoAntigo: v})} />
              <Input label="URL da Imagem" icon={<ImageIcon size={18}/>} placeholder="https://..." value={form.imagemUrl} onChange={(v) => setForm({...form, imagemUrl: v})} />
              <Input label="Link de Afiliado" icon={<LinkIcon size={18}/>} placeholder="https://..." value={form.urlAfiliado} onChange={(v) => setForm({...form, urlAfiliado: v})} />
              <Input label="Cupom (Opcional)" placeholder="EX: BARATINHO10" value={form.cupom} onChange={(v) => setForm({...form, cupom: v})} />
            </div>
